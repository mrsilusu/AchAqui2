...
function resolveSslMode() {
    if (process.env.DATABASE_SSLMODE)
        return process.env.DATABASE_SSLMODE;
...
var PrismaService = function () {
    ...
    var PrismaService = _classThis = /** @class */ (function (_super) {
        ...
        function PrismaService_1() {
            var runtimeDbUrl = withSslMode(process.env.DATABASE_URL);
            return _super.call(this, __assign(__assign({}, (runtimeDbUrl
                ? {
                    datasources: {
                        db: {
                            url: runtimeDbUrl,
