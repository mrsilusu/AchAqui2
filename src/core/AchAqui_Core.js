 ...
 *   - AppContext: Auth, PublicData e PrivateOwner contexts separados
 *
 * SEGURANÇA (v2.10.1):
 *   ✅ stripPrivateFields()   — remove campos sensíveis antes de expor ao contexto público
 *   ✅ sanitizeInput()        — strip HTML/JS/control chars em todos os inputs de texto
 *   ✅ clearSessionState()    — limpa estado sensível em toggleMode e logout
 *   ✅ tenantId binding       — RBAC via AuthContext.tenantId (nunca via prop)
 *   ✅ AsyncStorage namespace — chaves prefixadas com userId para isolamento
 *   ✅ URL validation         — validateExternalUrl() antes de Linking.openURL
...
export function Icon({ name, size = 20, color = '#1F1F1F', strokeWidth = 1.5 }) {
  ...
  const icons = {
    ...
    power:      <G {...base}><Path d="M18.36 6.64a9 9 0 1 1-12.73 0"/><Line x1="12" y1="2" x2="12" y2="12"/></G>,
...
export const PRICE_FILTERS = [
  ...
  { id: '1',   label: 'Económico',      levels: [1]       },
  { id: '2',   label: 'Moderado',       levels: [1,2]     },
  { id: '3',   label: 'Elevado',        levels: [1,2,3]   },
  { id: '4',   label: 'Luxo',           levels: [1,2,3,4] },
];

export const DISTANCE_FILTERS = [
  { id: 'all', label: 'Qualquer distância', max: 999 },
  { id: '1',   label: 'Até 1 km',           max: 1   },
  { id: '2',   label: 'Até 2 km',           max: 2   },
  { id: '5',   label: 'Até 5 km',           max: 5   },
  { id: '10',  label: 'Até 10 km',          max: 10  },
];
...
/**
 * stripPrivateFields — remove campos sensíveis de um objecto business
...
const PRIVATE_FIELDS = [
  'commissionRate', 'icalLink', 'icalSync', 'icalUrl',
  'autoConfirmBookings', 'invoicePrefix', 'bankDetails',
  'vatNumber', 'internalNotes', 'staffNotes',
];

export function stripPrivateFields(business) {
  if (!business || typeof business !== 'object') return business;
  const safe = { ...business };
  PRIVATE_FIELDS.forEach(f => delete safe[f]);
  // Sanitizar roomTypes: remover campos financeiros internos
  if (Array.isArray(safe.roomTypes)) {
    safe.roomTypes = safe.roomTypes.map(room => {
      const { taxRate, seasonalRates, bookedRanges, ...publicRoom } = room;
      return publicRoom;
    });
  }
  return safe;
...
/**
 * sanitizeInput — limpa texto de entrada externo (pesquisa, campos de texto livre,
 * dados de Google/Outscraper) antes de armazenar no estado ou enviar à API.
 *
 * PROTEGE CONTRA: XSS, HTML injection, JS protocol injection, control chars.
 * FASE 2+: O backend NestJS tem class-validator como segunda linha de defesa.
 */
export function sanitizeInput(raw, maxLen = 150) {
  if (typeof raw !== 'string') return '';
  return raw
    .slice(0, maxLen)
    .replace(/<[^>]*>/g, '')              // strip HTML tags
    .replace(/&[a-z]+;/gi, '')           // strip HTML entities
    .replace(/[\x00-\x08\x0B\x0E-\x1F\x7F]/g, '') // strip control chars (preserva \n \t)
...
/**
 * validateExternalUrl — valida URLs externas antes de Linking.openURL.
 * Previne Open Redirect e Deep Link Hijacking.
 *
 * Retorna true apenas para URLs https:// com domínio válido.
 * Nunca abre javascript:, data:, tel: sem verificação explícita.
 */
export function validateExternalUrl(url, allowTel = false) {
  if (!url || typeof url !== 'string') return false;
  const trimmed = url.trim().toLowerCase();
  if (trimmed.startsWith('https://')) return true;
  if (allowTel && trimmed.startsWith('tel:+')) return true;
  if (allowTel && trimmed.startsWith('tel:00')) return true;
  return false;
}

/**
 * sanitizeBusinessData — aplica stripPrivateFields + sanitização de campos
 * de texto provenientes de Google Places / Outscraper antes de entrar no estado.
 *
 * Campos sanitizados: name, description, address, neighborhood, hours, phone, website.
 * URLs de fotos: validadas para https:// antes de serem incluídas.
 */
export function sanitizeBusinessData(business) {
  if (!business || typeof business !== 'object') return business;
  const stripped = stripPrivateFields(business);
  return {
    ...stripped,
    name:         sanitizeInput(stripped.name, 100),
    description:  sanitizeInput(stripped.description, 500),
    address:      sanitizeInput(stripped.address, 200),
...
//
// SEGURANÇA v2.10.1:
//   ✅ businesses[] — apenas dados públicos (stripPrivateFields aplicado)
//   ✅ tenantId     — ID do negócio do dono (nunca alterável via prop)
//   ✅ AsyncStorage — chaves prefixadas com userId para isolamento
//   ✅ clearSessionState() — chamado em toggleMode e logout
//   ✅ updateOwnerBiz — aplica stripPrivateFields em dados públicos
//
// Estado específico de módulos (reservas, room editor, etc.) NÃO entra aqui.
// Fica em cada módulo isolado (HospitalityModule, BeautyModule, etc.)
// ─────────────────────────────────────────────────────────────────────────────

const DEFAULT_LOCATION = 'Talatona, Luanda';
...
export function AppProvider({ children, initialBusinesses }) {
  ...
  // ── RBAC — tenantId é imutável durante a sessão ────────────────────────────
  // FASE 2+: vem do JWT decode após login; nunca alterável via props ou state
  // Usado pelo BusinessEngine para verificar ownerMode independentemente da prop
  const tenantId = MOCK_TENANT_ID;
  const userId   = MOCK_USER_ID;

  // ── Dados públicos ─────────────────────────────────────────────────────────
  // SEGURANÇA: businesses[] contém APENAS dados públicos (sem campos privados)
  // stripPrivateFields() aplicado a cada business na inicialização
  // FASE 2+: substituir por useQuery(['businesses'], fetchBusinesses)
  // O backend devolve BusinessPublicDTO já sanitizado — stripPrivateFields actua
  // como guard adicional no frontend
  const [businesses, setBusinesses] = useState(() =>
    (initialBusinesses || []).map(sanitizeBusinessData)
  );

  // ── Owner private data — SEPARADO do contexto público ─────────────────────
  // O objecto OWNER_BUSINESS completo (com campos privados) vive aqui,
  // NUNCA é exposto no value do AppContext directamente.
  // Acessível APENAS através de useOwnerContext() quando isBusinessMode === true
  // FASE 2+: vem de GET /businesses/:id/private com JWT Bearer token
  const [ownerBusinessPrivate, setOwnerBusinessPrivate] = useState(OWNER_BUSINESS);

  /**
   * updateOwnerBiz — sincroniza campos do negócio do dono
   *
   * Para dados PÚBLICOS (nome, horário, fotos...): aplica stripPrivateFields
   * Para dados PRIVADOS (roomTypes.taxRate, seasonalRates...): actualiza ownerBusinessPrivate
   *
   * FASE 2+: substituir por useMutation + invalidate query
   */
  const updateOwnerBiz = useCallback((fields) => {
    // Actualiza dados privados completos
    setOwnerBusinessPrivate(prev => ({ ...prev, ...fields }));
    // Actualiza dados públicos SANITIZADOS em businesses[]
    setBusinesses(prev => prev.map(b =>
      b.id === OWNER_BUSINESS.id
        ? sanitizeBusinessData({ ...b, ...fields })
        : b
    ));
  }, []);
  ...
  // ── Pesquisa ──────────────────────────────────────────────────────────────
  const [searchWhat, setSearchWhatRaw] = useState('');
  const [searchWhere, setSearchWhere]  = useState(DEFAULT_LOCATION);
  // SEGURANÇA: sanitizar input antes de armazenar
  const setSearchWhat = useCallback((raw) => setSearchWhatRaw(sanitizeInput(raw, 100)), []);

  // ── Bookmarks — namespace por userId ──────────────────────────────────────
  // SEGURANÇA: chave prefixada com userId para isolamento entre utilizadores
  // no mesmo dispositivo
  const [bookmarkedIds, setBookmarkedIds] = useState([]);
  const bookmarkKey = `bookmarks_${userId}`;

  useEffect(() => {
    AsyncStorage.getItem(bookmarkKey)
      .then(s => { if (s) setBookmarkedIds(JSON.parse(s)); })
      .catch(() => {});
  }, [bookmarkKey]);
  ...
  const toggleBookmark = useCallback(async (id) => {
    setBookmarkedIds(prev => {
      ...
      AsyncStorage.setItem(bookmarkKey, JSON.stringify(updated)).catch(() => {});
      return updated;
    });
  }, [bookmarkKey]);
  ...
  // ── PII Delivery Registry — registo de callbacks de limpeza de extensões ───
  // Cada extensão (DeliveryExtension, etc.) pode registar o seu clearPII aqui.
  // clearSessionState chama todos ao fazer logout/toggleMode.
  // PADRÃO: módulos nunca guardam PII no AppContext — ficam em estado local
  // e registam apenas um callback de limpeza aqui.
  const piiCleanupCallbacks = useRef([]);
  const registerPIICleanup = useCallback((fn) => {
    piiCleanupCallbacks.current.push(fn);
    // Retorna função de deregisto (chamada no useEffect cleanup do módulo)
    return () => {
      piiCleanupCallbacks.current = piiCleanupCallbacks.current.filter(f => f !== fn);
    };
  }, []);

  // ── clearSessionState — chamado ANTES de qualquer troca de modo ────────────
  // SEGURANÇA: garante que dados sensíveis da sessão anterior não persistem
  // Cobre: bookmarks, notificações, pesquisa, histórico de navegação,
  //        PII de entrega (endereços) registados pelas extensões
  const clearSessionState = useCallback(() => {
    setBookmarkedIds([]);
    setNotifications([]);
    setSearchWhatRaw('');
    setSearchWhere(DEFAULT_LOCATION);
    // PII WIPE: limpar endereços e dados pessoais de todas as extensões activas
    piiCleanupCallbacks.current.forEach(fn => { try { fn(); } catch {} });
    piiCleanupCallbacks.current = [];
    // Remove bookmarks do userId anterior do AsyncStorage
    AsyncStorage.removeItem(bookmarkKey).catch(() => {});
  }, [bookmarkKey]);

  // ── Toggle modo dono ──────────────────────────────────────────────────────
  // SEGURANÇA: clearSessionState() chamado ANTES de trocar modo
  const toggleBusinessMode = useCallback(() => {
    clearSessionState(); // ← limpa estado da sessão anterior
  ...
  // logout — FASE 2+: chamar DELETE /auth/session + limpar JWT
  ...
  const value = {
    ...
    tenantId,   // ← RBAC: ID do negócio do dono (imutável, vem de JWT)
    userId,     // ← para namespace de AsyncStorage
    locationPermission, requestLocationPermission,
    // Dados PÚBLICOS (sanitizados — sem campos privados)
    businesses, setBusinesses, updateOwnerBiz,
    // Dados PRIVADOS do dono (apenas acessíveis via isBusinessMode check)
    // NOTA: expor ownerBusinessPrivate aqui permite que qualquer consumidor
    // do context o aceda. Em produção, usar PrivateOwnerContext separado.
    // Para Fase 1 (mock data), esta abordagem é aceitável.
    ownerBusinessPrivate,
    // Navegação
    activeNavTab, setActiveNavTab,
    activeBusinessTab, setActiveBusinessTab,
    // UI
    darkMode, toggleDarkMode,
    // Search (input sanitizado automaticamente)
    ...
    COLORS, SCREEN_WIDTH,
    // PII — registo de callbacks de limpeza (extensões de entrega)
    registerPIICleanup,
  };
...
const navStyles = StyleSheet.create({
  bar:          { flexDirection: 'row', backgroundColor: '#FFFFFF', borderTopWidth: 1, borderTopColor: '#EBEBEB' },
  tab:          { flex: 1, alignItems: 'center', paddingTop: 10, gap: 4 },
  tabLabel:     { fontSize: 10, color: '#8A8A8A', fontWeight: '500' },
  tabLabelActive:{ color: '#D32323', fontWeight: '700' },
  badge:        { position: 'absolute', top: -4, right: -6, backgroundColor: '#D32323', borderRadius: 8, minWidth: 16, height: 16, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 2 },
      ...
      <View style={headerStyles.searchRow}>
        <View style={headerStyles.searchField}>
          ...
          <TextInput
            ...
            value={searchWhat}
            onChangeText={setSearchWhat}  // ← sanitizeInput() aplicado no setter
...
const headerStyles = StyleSheet.create({
  wrapper:       { backgroundColor: '#FFFFFF', paddingHorizontal: 16, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: '#EBEBEB' },
  topRow:        { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 },
  logoWrap:      { flexDirection: 'row', alignItems: 'baseline', gap: 6 },
  logo:          { fontSize: 22, fontWeight: '800', color: '#D32323', letterSpacing: -0.5 },
  version:       { fontSize: 11, color: '#8A8A8A', fontWeight: '500' },
  actions:       { flexDirection: 'row', gap: 4 },
  actionBtn:     { width: 36, height: 36, borderRadius: 18, backgroundColor: '#F7F7F8', alignItems: 'center', justifyContent: 'center' },
  notifBadge:    { position: 'absolute', top: -4, right: -4, backgroundColor: '#D32323', borderRadius: 8, minWidth: 16, height: 16, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 2 },
  notifBadgeText:{ fontSize: 9, color: '#FFFFFF', fontWeight: '800' },
  searchRow:     { flexDirection: 'row', backgroundColor: '#F7F7F8', borderRadius: 12, borderWidth: 1, borderColor: '#EBEBEB', overflow: 'hidden' },
  searchField:   { flex: 1, paddingHorizontal: 12, paddingVertical: 8 },
  searchLabel:   { fontSize: 9, fontWeight: '700', color: '#8A8A8A', letterSpacing: 0.8, marginBottom: 2 },
  searchInput:   { fontSize: 13, color: '#111111', padding: 0 },
  searchDivider: { width: 1, backgroundColor: '#EBEBEB', marginVertical: 8 },
});
...
// ─────────────────────────────────────────────────────────────────────────────
// FILTROS & ORDENAÇÃO — v2.11.0 (Fase 3: centralizados desde Main)
// ─────────────────────────────────────────────────────────────────────────────

export const TRENDING_SEARCHES = [
  'Pizza', 'Cafe', 'Farmacia 24h', 'Restaurante angolano', 'Barbearia',
];

export const AUTOCOMPLETE_SUGGESTIONS = [
  'Pizzaria', 'Pizza delivery', 'Pastelaria', 'Farmacia', 'Farmacia 24h',
  'Cafe', 'Cafeteria', 'Restaurante', 'Restaurante italiano',
  'Restaurante angolano', 'Barbearia', 'Supermercado',
];
...
export const AMENITY_FILTER_CATEGORIES = [
...
export const AMENITY_ICON_MAP = {
  ...
  wheelchair: 'wheelchair', kids: 'users',         livemusic: 'star',
  tpa: 'payment',       homedelivery: 'delivery', '24h': 'clock',
  ac: 'certified',      carts: 'delivery',         appointment: 'calendar',
  courtesy: 'heart',    shower: 'outdoor',         certified: 'certified',
  online: 'web',        freeconsult: 'heart',      portfolio: 'portfolio',
  languages: 'globe',   generator: 'certified',    reservations: 'reservation',
  petfriendly: 'heart', remote: 'remote',          fastdelivery: 'fastdelivery',
  flexible: 'clock',    professional: 'professional',
};

// ─────────────────────────────────────────────────────────────────────────────
// OPERATIONAL MODULES — v2.9.1 (configuração dos módulos operacionais)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// UTILITY FUNCTIONS — lógica de negócio partilhada
// ─────────────────────────────────────────────────────────────────────────────

/**
 * getBusinessStatus — calcula estado de fecho iminente a partir do statusText.
 * @returns {{ isClosed: boolean, minsLeft: number|null }}
 */
export const getBusinessStatus = (statusText, isOpen) => {
...
/**
 * renderHeroStars — estrelas para fundos escuros (hero foto).
 * Activa: #FCD34D, inactiva: rgba branco semi-opaco.
 */
export const renderHeroStars = (rating, size = 11) => {
  const stars = [];
  for (let i = 1; i <= 5; i++)
    stars.push(<Text key={i} style={{ fontSize: size, color: i <= rating ? '#FCD34D' : 'rgba(255,255,255,0.35)', marginRight: 1 }}>★</Text>);
  return <View style={{ flexDirection: 'row', alignItems: 'center' }}>{stars}</View>;
};

// ─────────────────────────────────────────────────────────────────────────────
// NAV_BAR_STYLES — bottom navigation bar (cliente + negócio)
...
export const NAV_BAR_STYLES = StyleSheet.create({
  ...
  label:         { fontSize: 9, fontWeight: '600', color: '#8A8A8A', letterSpacing: 0 },
  labelActive:   { color: '#D32323', fontWeight: '700' },
});
